...
class Logger:
    ...
    def shouldPrintMessage(self, timestamp: int, message: str) -> bool:
        ...
        if message not in self.dict:
            self.dict[message]=timestamp #add message and timestamp in dict, if it's not already there.
            return True
        elif self.dict[message]+10 <= timestamp: # if it's already there, is the current timestamp >= the last time stamp+10
